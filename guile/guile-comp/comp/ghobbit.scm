...
;   and redistribution of hobbit version 4d are given in the file COPYING
; 
;
...
(if *expand-macros-flag*
    (begin (require 'defmacroexpand)(require 'pprint-file)) )
...
s-make-vector s-list s-string s-vector repl-driver newsmob scm_dynthrow repl
...
;;; makfromstr
 ...
 makarb tryarb relarb ceval scm-error
 prolixity gc gc-for-newcell tryload cons2
...
stci-lessp substring st-append vectorp make-vector
 ...
 call-cc copytree
;;; eval
 thrown_value quit input-portp output-portp
...
(define (hobbit file . files)
  (let* ((tmpname "hobbit.tmp"))
    ...
    ; check for defmacros 
    (if *expand-macros-flag*
        (if (not (some (lambda(x)(file-contains-defmacro? x))
                       (cons file files) ))
            (set! *expand-macros-flag* #f) ))
    (set! *initial-defs* '())
    (if *expand-macros-flag* (for-each load (cons file files)))
    (for-each (lambda(x)
...
(define (expr-contains-defmacro? expr)
  (cond ((not (pair? expr)) #f)
        ...
        (else (pair-some (lambda(x)(expr-contains-defmacro? x)) expr)) ))
...
(define (compile-defs file deflst)
  (let ()
    ...
    (if (some (lambda(x)(in-fun-position? 'delay x)) deflst)
	(set! deflst
	      (append deflst
		      (append
   ...
   "These primitive procedure names are assumed to be redefinable (slow):")
	     ...
	     (display *modified-primitives*)
	     (newline) ))
       (if (not (null? (difference *modified-top-level-names* 
				 (set-union
   ...
   "These top level procedure names are assumed to be redefinable (slow):")
	     (newline) 
	     (display (difference *modified-top-level-names* 
				 (set-union
				  *global-vars-list* *fast-vars-list*) ))
	     (newline) ))))
    ; - - - - - - - - - hof-info: - - - - - - - - - - - - - -
   ...
   "These top level procedures create non-liftable closures (slow):")
    ...
    (if (and *infomessages-flag*
             (not (null?  (difference (difference *unknown-functions*
						  *modified-top-level-names*)
				      (set-union *global-vars-list*
					     (set-union *fast-vars-list*
 ...
 "These nonprimitive procedures are assumed to be defined externally:")
	  (newline)
	  (display 
	   (difference (difference *unknown-functions*
				   *modified-top-level-names*)
		       (set-union *global-vars-list*
			      (set-union *fast-vars-list*
				     *via-interpreter-defined*))))
	  (newline) ))
    (if (and *infomessages-flag*
             (not (null? (difference (difference *unknown-vars*
						 *modified-top-level-names*)
				     *via-interpreter-defined*))))
	(begin (newline)
	  (display
   "These variables undefined (but used) in your file were defined:")
	  (newline)
	  (display (difference (difference *unknown-vars*
					   *modified-top-level-names*)
			       *via-interpreter-defined*))
	  (newline) ))
    (if *infomessages-flag* (newline)) ))
  ...
  (do ((part lst (cdr part)))
	...
	(cond
      ...
      (let* ((top-actions-fun 
		...
		(list* 'lambda '() (reverse *top-actions-list*)) )))
	(set! *to-do-fun-list* (cons top-actions-fun *to-do-fun-list*)) )
	...
	(let ((tmp (member-if (lambda(x)
	  ...
	  (if tmp
	      ...
	      (set! *inline-vars* (delete (car part) *inline-vars*)) )))
		    ...
		    (,*c-adr*
			  ...
			  (,*intern-function*
				...
				,(symbol->string *constant-list-var*) )
			     ,(string-length
				 (symbol->string *constant-list-var*) )))))
	     `(set! (,*global-access* ,*constant-list-var*) ()) ))
	  (init-all
	   (list 'define
...
(define (init-export-funs! file)
  (let* ((res '())
	 ...
	 (topactions-fun 
	  (car (member-if (lambda(x)(eq? (cadr x) topactions-funname))
			  *passed-defs*))))
    (set! *export-functions*
    ...
    (for-each
	 ...
	 (if res 
	     (subst-term-equal!
	      res (list *special-pseudoquote* (cadr x)) topactions-fun)))
       *passed-defs*)))
...
(define (display-header fport)
  ...
  (headerline "#include <guile/guilehob.h>")
  (headerline "") )
...
(define (write-fun-declaration var port)
   (set! *c-port* port)
   (if (not (string=? *init-fun-prefix* 
                      (substring var 0 (min
                                        (string-length var)
                                        (string-length *init-fun-prefix*)))))
       (display-c "static "))
   (display-c *scm-type*)
   (display-c #\space)
   (display-c-var var)
...
(define (write-c-wholefun def port)
  (let* ((fun (caddr def))
   ...
   (if (not (string=? *init-fun-prefix* 
                      (substring *current-fun-name* 0 (min
                                        (string-length *current-fun-name*)
                                        (string-length *init-fun-prefix*)))))
       (display-c "static "))
   (display-c *scm-type*)
   (display-c #\space)
   ...
   (if (and (not (null? (cadr top-let)))
	    (some (lambda(x)(symbol? (car x))) (cadr top-let)) )
       (begin
	 (display-c-indent 1)
	 (display-c *scm-type*)
	 (display-c #\space)
...
(define (type-const-pass term)
 (let ((res
	...
	"exact arithmetic assumed but a nonexact number encountered: " term))
	 ...
	 ((eq? (car term) 'let*)
	     (set! *local-vars* (set-union (map car (cadr term)) *local-vars*))
	     (cons 'begin
		   (map type-const-pass
		     (begins->list
...
(define (protect-constant x)
  (set! *new-constant-list*
	(cons `(set! (,*global-access* ,*constant-list-var*)
		     (cons ,x (,*global-access* ,*constant-list-var*)) )
	      *new-constant-list*)))
...
(define (lift-statements term checkvars)
   (cond
	...
	(lift-statements (push-result-var-in (cadr term) (caddr term))
			 checkvars))
      ((eq? 'do (car term))
	     (set! checkvars (set-union (map car (cadr term)) checkvars))
	     (list* 'do
		    (map
		       (lambda(x)
...
(define (lambda-lift term boundvars new-names-args)
   (let ((tmp '()))
	 ...
	 ((eq? (car term) 'lambda)
	      (set! tmp (set-union (args->list (cadr term)) boundvars))
	      (cons 'lambda
		    (cons (cadr term)
			  (map (lambda(x)
	 ...
	 ((eq? (car term) 'do)
	     ; check next line!!!
	     (set! tmp (set-union (map car (cadr term)) boundvars))
	     (cons 'do
		   (cons (map (lambda(x)
				(if (null? (cddr x))
				    (list (car x)
   ...
   (let* ((bindings (cadr letterm))
	  ...
	  (other-bindings
	    (filter (lambda(x)
		    ...
		    bindings))
	  (next-bound (set-union (map car other-bindings) boundvars)) )
   ...
   (cond ((not (pair? term)) term)
	 ...
	 ((memq (car term) '(let let* letrec))
	  ...
	  ((and (memq (car term) '(cond))
		(some (lambda(cl)
		        (some (lambda(x)(and (pair? x)(eq? 'lambda (car x))))
			      (cdr cl) ))
		      (cdr term) ))
	     (let* ((lcl
		     (some (lambda(cl)
			   ...
			   (cdr term) ))
		    (lterm
		      (some(lambda(x)(and(pair? x)(eq? 'lambda(car x)))) lcl) )
...
(define (occurrences-of vars term)
  (cond ((symbol? term)
	...
	((eq? (car term) 'quote) '())
	(else
	   (set-union (occurrences-of vars (car term))
...
(define (all-free-aux! term bound)
  (cond
	...
	(if (and (not (memq term bound))
     ...
     ((eq? 'quote (car term)))
     ((eq? 'lambda (car term))
	(let ((new (set-union (args->list (cadr term)) bound)))
	  (for-each (lambda(x)(all-free-aux! x new)) (cddr term)) ))
     ((eq? 'let (car term))
	(let ((new (set-union (map car (cadr term)) bound)))
     ...
     ((eq? 'letrec (car term))
	(set! bound (set-union (map car (cadr term)) bound))
	(for-each (lambda(x)(all-free-aux! (cadr x) bound)) (cadr term))
	(for-each (lambda(x)(all-free-aux! x bound)) (cddr term)) )
     ((eq? 'do (car term))
	(let ((new (set-union (map car (cadr term)) bound)))
...
(define (merge-free-vars pair-a pair-b)
   (let* ((norm-a (car pair-a))
	  ...
	  (set-a  (cadr pair-a))
	  (set-b  (cadr pair-b))
	  (set-res (set-union set-a set-b)) )
      (list (difference (set-union norm-a norm-b) set-res)
	    set-res )))
...
(define (liftable-hofvars-usage? term name hof-args args)
  (cond
    ...
    ((or (eq? 'lambda (car term)) (eq? *liftable-lambda* (car term)))
       (not (some (lambda(x)(some-inside-term? hof-args x)) (cddr term))) )    
    ((eq? name (car term))
       (all (lambda(x)
	      (let ((param (car x))
		    (arg (cdr x)) )
    ...
    (else
      (and (all (lambda (el) (not (memq el hof-args))) (cdr term))
           (all (lambda(x)(liftable-hofvars-usage? x name hof-args args))
		term)))))
...
;  (pretty-print expr-list)
  (set! *modified-primitives*
	(difference *modified-primitives* *general-transcedentals*) )
  *top-level-names*)
...
(define (make-closure-making-aux
  ...
  (cond 
   ...
   ((and (eq? 'letrec (car term))
	 ...
	 (all (lambda(el)(eq? 'lambda (caadr el)))
	       (cadr term) )
	 (some (lambda(el)(not (memq (car el) clvars)))
	       (cadr term) ))
...
(define (make-lifted-closure-applic definf holes)
  (let* ((vars (car definf))
	...
	(set! letrec-assignments
	      (filter (lambda(x)
			(member (cadddr x) holes) )
		      assignments)))
    (if (not (null? holes))
	(set! assignments
	      (filter (lambda(x)
			(not (member x letrec-assignments)) )
		      assignments)))
    (if (not (null? holes))
	...
	; closure does not occur in letrec top
	(if (null? assignments)
...
(define (inside-nonliftable-term? name term)
    (cond
      ...
      (else
        (some (lambda (x) (inside-nonliftable-term? name x)) term) )))
...
(define (beautify-closure term)
  (cond
   ...
   ((and (memq (car term) '(let* let letrec))
	 (not (null? (cdr term)))
	 (pair? (cadr term))
	 (some (lambda(x)(pair? (car x))) (cadr term)) )
...
(define (pair-some f lst)
  (if (pair? lst)
      (if (f (car lst)) (car lst) (pair-some f (cdr lst)))
      (if (f lst) lst #f) ))

(define (all f lst)
   (if (null? lst)
       #t
       (if (f (car lst)) (all f (cdr lst)) #f) ))
...
(define (some-inside-term? obs term)
  (cond ((memq term obs) #t)
	((pair? term)
	   (if (eq? 'quote (car term))
	       #f
	       (or (some-inside-term? obs (car term))
		   (some-inside-term? obs (cdr term)) )))
	(else #f) ))

(define (subst-term-equal! what for term)
  (if (pair? term)
      (if (equal? (car term) for)
	  (begin (set-car! term what)
		 (subst-term-equal! what for (cdr term)) )
	  (if (not (eq? 'quote (car term)))
	      (begin (subst-term-equal! what for (car term))
		     (subst-term-equal! what for (cdr term)) )))))
