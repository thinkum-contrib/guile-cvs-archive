...
#ifdef __STDC__
int
regncomp (regex_t * preg, int len, const char * pattern, int cflags)
#else
int
regncomp (preg, len, pattern, cflags)
     regex_t * preg;
     int len;
     const char * pattern;
     int cflags;
...
#endif
{
  ...
  return regncomp (preg, strlen (pattern), pattern, cflags);
}
...
#endif
{
  ...
  for (x = start; x <= end; ++x)
    {
      if (preg->is_nullable
	  || ((x < end) && (preg->fastmap[((unsigned char *)string)[x]])))
	{
...
#ifdef __STDC__
int
regnexec (const regex_t *preg, int len, const char *string, size_t nmatch, regmatch_t **pmatch, int eflags)
#else
int
